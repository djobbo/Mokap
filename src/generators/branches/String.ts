import ret, { Char, Group, Set, Range, Root, Tokens } from 'ret';
...
function genGroup(token: Root | Group, groups: string[] = [], options: IGenOptions): [string, string[]] {
  const stack = token.options ? randomValueFromArray(token.options) : token.stack || [];
...
function expandSet(token: Char | Range | Set, options: IGenOptions) {
  switch (token.type) {
    ...
    case SET: {
      ...
      return token.not ? options.defaultRange.clone().subtract(drange) : options.defaultRange.clone().intersect(drange);
    }
  }
}
